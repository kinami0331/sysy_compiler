...
bool ConstInitValNode::hasOnlyOneExp() {
    if (!isList)
        return true;
        // 如果是个列表，则为一个EXP元素的列表
    else if (childNodes.size() == 1) {
        return !((ConstInitValNode *) childNodes[0])->isList;
    }
    return false;
}

NodePtr ConstInitValNode::getTheSingleVal() {
...
//    vector<int> dims = ((CEInBracketsNode *) childNodes[1])->adjustArray();
    // 首先将初始化列表进行标准化
//    print();
    ((ConstInitValNode *) childNodes[2])->standardizing(dims);
}
...
void ConstInitValNode::standardizing(vector<int> &dims) {
    ...
    if (dimNum == 1) {
        for (int i = 0; i < childNum; i++)
            childNodes[i] = ((ConstInitValNode *) childNodes[i])->getTheSingleVal();
        return;
    }
    ...
    for (int i = 0; i < childNum; i++) {
        // 如果这个元素是列表，直接j += 1
        if (((ConstInitValNode *) childNodes[i])->isList) {
            ...
            // k是要寻找的列表数
            // newNode是这一轮找到的总列表
            NodePtr newNode = new ConstInitValNode(true);
            int k = curDimListNum;
            while (k > 0 && i < childNum) {
                ...
                if (!((ConstInitValNode *) childNodes[i])->isList) {
                    ...
                    for (ii = i; ii < childNum && ii < i + lastDim; ii++) {
                        assert(((ConstInitValNode *) childNodes[ii])->hasOnlyOneExp());
                        newNode->pushNodePtr(((ConstInitValNode *) childNodes[ii])->getTheSingleVal());
                    }
    ...
    for (int i = 0; i < curDim - j; i++) {
        NodePtr t = new ConstInitValNode(true);
    ...
    for (auto &childNode : childNodes) {
        ((ConstInitValNode *) childNode)->standardizing(nextDims);
    }
}
...
void ConstInitValNode::adjust(vector<int> &dims) {
